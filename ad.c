...
int typeSize(Type *t){
	if(t->n<0)return typeBaseSize(t);//tablou de dim fixa
	if(t->n==0)return sizeof(void*);//dimensiunea unui pointer
	return t->n*typeBaseSize(t);//tablou de dim variabila
	}
